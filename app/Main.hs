...
main = do
  ...
  let o = [int, uni, sub, st] ++ map (\i -> sphere (i *. ex) 0.3 yellow) [0 .. 100]
  let f = 600
  forM_ [0 ..] $ \i -> do
    start <- getCPUTime
    let !can = let tmp = canvas (lookat (rot ey (10 *. ex) (i / f * 2 * pi)) e0) (100, 100) (View 1 1) o in tmp `deepseq` tmp
    ecan <- getCPUTime
    let !disp = let tmp = dispc can in tmp `deepseq` tmp
    edis <- getCPUTime
    T.putStrLn disp
    end <- getCPUTime
